...
(λ M.string-format [fmt sub-args]
...
(λ M.keyword->query [hl keywords]
  ...
  (let [hl (.. "@comment." hl)
    ...
    (M.string-format M.template {: hl : keywords})))

(λ M.keywords->query [keywords]
  "Convert an array of keywords into a query."
  (let [keywords (icollect [hl keyword (pairs keywords)]
                   (M.keyword->query hl keyword))]
    (table.concat keywords "\n")))

(λ M.apply-queries [keywords]
  (let [hl-query (.. (M.keywords->query keywords) M.other-queries)]
